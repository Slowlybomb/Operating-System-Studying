...
main:				# main, a bit like init/boot loader
	...
	li $a1, 0		# $a1 contains number of qunata the process will run for before switching
...
.kdata
pcb:	.word  0 : 35 		# each Process Control Block (PCB) has PC, a0, v0, t0, t1, priority, counter  
...
priority_temporary: .word	# This whill be used to temporarily store the priority counter of a process in the interrupt handler
...
s100_next:	
	...
	bne   $k0,140, s100_next# we can have up to 5 pcb (5 times 30 byte)
...
continue_execution:
	sw    $k1, priority_temporary # Store priority counter in temporary location
...
int_hdlr: #clock used to switch to next process (we do not check exact interrupt)
	...
	lw    $k1, curpcb   	# load the offest to current pcb (in multiples of 28)
	add   $k0, $k0, $k1 	# get address of the current active pcb
	lw    $k1, 24($k0)	# load priority counter
	lw    $k0, 20($k0)	# load priority, overwriting $k0
	addi  $k1, $k1, 1	# increment priority counter
	ble   $k1, $k0, continue_execution # if the program hasn't executed it's designated quanta, resume execution
	
	la    $k0, pcb	 	# load start address of the pcb array
	lw    $k1, curpcb   	# load the offest to current pcb (in multiples of 28)
	add   $k0, $k0, $k1 	# get address of the current active pcb   
	li    $k1, 0		# reset priority counter
	sw    $k1 24($k0)
	...
	lw    $k0, curpcb 	# load current pcb offset
int_next:
	addiu $k0, $k0, 28	# add 35 to get to next pcb
